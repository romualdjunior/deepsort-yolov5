...
def export_engine(model, im, file, train, half, simplify, workspace=4, verbose=False, prefix=colorstr('TensorRT:')):
    ...
    try:
        ...
        assert im.device.type != 'cpu', 'export running on CPU but must be on GPU, i.e. `python export.py --device cpu`'
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    assert not (device.type == 'cpu' and half), '--half only compatible with GPU export, i.e. use --device cpu'
